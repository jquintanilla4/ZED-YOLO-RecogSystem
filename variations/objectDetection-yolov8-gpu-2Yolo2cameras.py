...
# 2 YOLOv8 instances on GPU, one for each ZED camera
...
camera_index_list = [38845823, 37771754]
# number of cameras
...
# Create a list of run signals for each camera
run_signal_list = [False for _ in range(num_cameras)]
# Create a list of exit signals for each camera
exit_signal_list = [False for _ in range(num_cameras)]
...
def torch_thread(weights, img_size, conf_thres=0.7, iou_thres=0.7, i=0): # conf originally 0.4, iou originally 0.45
    global class_names, yolo_output_label
    try:
        ...
        while not exit_signal_list[i]:
            # print(f"Torch Thread running for camera {camera_index}") # debugging
            # When the run signal is true, acquire the lock to prevent simultaneous access to shared resources
            if run_signal_list[i]:
                # Acquire the lock
                lock_list[i].acquire()
...
def main_loop(i, svo_filepath=None):
    ...
    # Detect objects loop
    while not exit_signal_list[i]:
        # print(f"Main loop running for camera {camera_index}") # debugging
        ...
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            run_signal_list[i] = True

            # Wait for detections on the other thread
            while run_signal_list[i]:
                sleep(0.001)
            ...
            # print(f"Creating openCV window for camera {camera_index}") # debugging
            cv2.imshow(f"2D View Camera {camera_index}", image_left_ocv)
            # This allows OpenCV to process its event queue
            cv2.waitKey(10)

            # print(f"Main loop finished for camera {camera_index}") # debugging

            # If 'q' is pressed, exit the loop
            if keyboard.is_pressed('q'):
                exit_signal_list[i] = True
                break
...
def main():
    ...
    for i in range(num_cameras):
        main_thread = Thread(target=main_loop, args=(i, opt.svo))
        main_thread.start()
